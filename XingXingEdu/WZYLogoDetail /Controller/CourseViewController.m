...
//    _schoolIdStr = schoolIdStr;
    ...
    NSDictionary *params = @{@"appkey":APPKEY, @"backtype":BACKTYPE, @"xid":XID, @"user_id":USER_ID, @"user_type":USER_TYPE, @"school_id":schoolIdStr};
...
- (void)createImageView{
    ...
    [_iconImageView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",picURL, _logoIconStr]] placeholderImage:[UIImage imageNamed:@"头像174x174@2x.png"]];
...
//
    ...
    if ([_teacherNameArray[indexPath.row] count] != 0) {
        ...
        for (int i = 0; i < [_teacherNameArray[indexPath.row] count]; i++) {
            UILabel *teacherNameLabel = [HHControl createLabelWithFrame:CGRectMake((190 + 70 * i) * kWidth / 375, 35 , 60 * kWidth / 375, 20) Font:14 * kWidth / 375 Text:_teacherNameArray[indexPath.row][i]];
