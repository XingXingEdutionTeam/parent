...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tongzhi:) name:@"deleteCommment" object:nil];
}
...
//    NSLog(@"==============DFBaseLineItem===================%ld",item.userId);
...
- (void)tongzhi:(NSNotification *)text{
    ...
    DFBaseLineItem *item = [self getItem:[text.userInfo[@"textThree"] integerValue]];
    

    for (int i = 0 ; i < item.comments.count; i ++) {
        if (text.userInfo[@"textFour"] == item.comments[i]) {
            
            [item.comments removeObjectAtIndex:i];
        }
    }


    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"deleteCommment" object:self];
...
-(void)onClickUser:(NSUInteger)userId{
    //没有调用
    NSLog(@"================onClickUseronClickUser=====================>%ld",(unsigned long)userId);
}
//回复
