...
@property (nonatomic, strong) UILabel *timeLabel;//cell上的时间

@property (nonatomic, strong) UIButton *likeCmtButton;//点赞
...
-(void) updateLocationLikeComment:(CGFloat)height{
    ...
    if (self.item.likes.count == 0 && self.item.comments.count == 0) {
        ...
        [_likeCommentView setNeedsDisplay];
    }
    //点赞和评论
    if (self.item.likes.count == 0) {
        
        _likeCommentToolbar.likeButton.selected = NO;
       [_likeCommentToolbar.likeButton setTitle:@"赞" forState:UIControlStateNormal];
       ...
       [_likeCommentToolbar.likeButton setTitle:@"取消" forState:UIControlStateNormal];
    }
