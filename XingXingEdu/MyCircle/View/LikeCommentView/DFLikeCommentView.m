...
-(void)updateWithItem:(DFBaseLineItem *)item
{
    ...
    if (item.comments.count > 0) {
        ...
        for (int i=0;i<item.comments.count;i++) {
            ...
            label.talk_Id = commentItem.talk_id;
            label.commont = commentItem;
    ...
    [mgr POST:urlStr parameters:dict success:^(AFHTTPRequestOperation *operation, id responseObject)
     {
         ...
         if([[NSString stringWithFormat:@"%@",dict[@"code"]]isEqualToString:@"1"] )
         {
             ...
             NSDictionary *dict =[[NSDictionary alloc] initWithObjectsAndKeys:[NSString stringWithFormat:@"%lld", label.uniqueId],@"textOne",label.text,@"textTwo",label.talk_Id,@"textThree",label.commont,@"textFour", nil];
             //创建通知
             NSNotification *notification =[NSNotification notificationWithName:@"deleteCommment" object:nil userInfo:dict];
             //通过通知中心发送通知
