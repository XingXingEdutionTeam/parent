...
//    self.edgesForExtendedLayout = UIRectEdgeNone;
    ...
    [self createMessageHistoryNetRequest];
}
-(void)createMessageHistoryNetRequest{
    _dataArr = [NSMutableArray array];
    
    NSString *urlStr = @"http://www.xingxingedu.cn/Global/circle_history_msg";
         ...
         if([[NSString stringWithFormat:@"%@",dict[@"code"]] isEqualToString:@"1"] ){
             ...
             if ([listArr count] > 0) {
                 for (NSDictionary *dic in listArr) {
                     ...
                     NSString *head_img =dic[@"head_img"];
                     ...
                     NSMutableArray *arr =[NSMutableArray arrayWithObjects:head_img, talk_id,con,nickName,date_tm,head_img_type,nil];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([tmp[5] isEqualToString:@"0"]) {
        [headImage sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",picURL,tmp[0]]] placeholderImage:nil];
