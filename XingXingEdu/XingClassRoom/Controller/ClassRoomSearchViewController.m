...
- (void)createUI
{
    ...
    //历史记录tableview
    self.hisTV = [[UITableView alloc]initWithFrame:CGRectMake(90,CGRectGetMaxY(self.inputTF.frame),[UIScreen mainScreen].bounds.size.width - 150,100 )];
...
//    self.hisTV.clipsToBounds = false;
    ...
    [self.view addSubview:self.relationCombox];
     ...
     [self.hisTV  bringSubviewToFront:self.searchView];
}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    if (self.hisArr &&self.hisDicArr.count>0) {
        //搜索历史显示
        [self getData];
