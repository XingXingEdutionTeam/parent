...
@interface HomepageViewController ()<UIScrollViewDelegate, ZJCircularBtnDelegate>
{
...
//    self.schoolNameCombox.textField.placeholder = SCHOOL_NAME;
    ...
    [[NSUserDefaults standardUserDefaults] setObject:_schoolId forKey:@"SCHOOL_ID"];
    ...
    [[NSUserDefaults standardUserDefaults] setObject:_classStr forKey:@"CLASS"];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if ([str integerValue] == 1) {
        if ([object isKindOfClass:[UITextField class]]){
            ...
            NSString *classId = arr[didSelectedClassRow];
...
//        NSLog(@"学校 数组 %@", commboxSchoolArray);
//        NSLog(@"班级 %@", commboxGradeAndClassArray[0][0]);
...
{
    NSString *nameStr =[[NSUserDefaults standardUserDefaults]objectForKey:@"TT"];
...
//        [landVC  popoverPresentationController];
//        [landVC dismissViewControllerAnimated:YES completion:nil];
